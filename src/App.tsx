...
import { PatternEditor } from './components/playlist/patterns/editor/PatternEditor';
import { PianoRoll } from './components/pianoroll/PianoRoll';
import { useProjectStore } from './states/ProjectState';
import { PatternDefinition } from './models/models';
import { PlayList } from './components/playlist/PlayList';
import { SQButton } from './components/ui/SQButton';
...
function App() {
  ...
  const getPattern = (id: string) => project.patterns.find(p=>p.id===id)!;
  const { title } = project;
  ...
  return (
    <>
      <div style={{position: 'fixed', right: 0, top: 0, padding: '8px', backgroundColor: 'black', zIndex: 999, borderBottomLeftRadius: 7, display: 'flex', gap: '8px'}}>
      <SQButton variant="accent" onClick={initializeEngine}>Iniciar</SQButton>
      <SQButton disabled={!initialized} onClick={playNote}>
        Test note
      </SQButton>
      </div>
      { project.editingPattern &&
        <PatternEditor pattern={getPattern(project.editingPattern)}>
